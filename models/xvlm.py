...
from models.vm import VmTransformer, im_pos_embed
...
class XVLMBase(nn.Module):
    def __init__(self, config=None, load_vision_params=False, load_text_params=False,
        ...
        self.fc = nn.Linear(vm_width, vm_width)
    ...
    def get_matching_loss(self, image_embeds, image_atts, image_feat, text_embeds, text_atts, text_feat, idx=None, 
        ...
        for i in range(bs):
            for j in range(i+1, bs):
                ...
                text_single_sims[i][j] = torch.sqrt(torch.sum((text_single_sims[i] - text_single_sims[j]) ** 2))  
...
#         loss_i2t_asym = F.cross_entropy(sims_i2t_fusion_asym , labels_i2t_asym)
        ...
        return (loss_t2i+loss_i2t), loss_text_KD
...
#         bs = image_embeds.size(0)
#         cross_pos_asym = self.vm_encoder(image_embeds, text_embeds=text_embeds, text_atts=text_atts, image_atts=image_atts)
...
#         return loss_mpr
        ...
        bs = image_embeds.size(0)
#         cross_pos_asym = self.get_cross_embeds(image_embeds, image_atts, text_embeds,text_atts)[:, 0,:]
        cross_pos_asym = self.vm_encoder(image_embeds, text_embeds=text_embeds, text_atts=text_atts, image_atts=image_atts)
        ...
        mpr_matric = mpr_matric.to(image_embeds.device)
        mpr_embed = image_embeds * mpr_matric
#         mpr_pos_asym = self.get_cross_embeds(image_embeds, image_atts, text_embeds,text_atts)[:, 0,:]
        mpr_pos_asym = self.vm_encoder(mpr_embed, text_embeds=text_embeds, text_atts=text_atts)
